...
SmartFoxClient = function(objRef)
{
	...
	this.arrayTags		= { userList:true, rmList:true, vars:true, bList:true }
...
SmartFoxClient.prototype.handleSysMessages = function(xmlObj, parent)
{
	...
	else if (action == "bUpd")
	{
		...
		{
			if (parent.buddyList[i].name == name)
			{
...
//-----------------------------------------------------------------------------------//
SmartFoxClient.prototype.createRoom = function (roomObj)
{
	var roomId 		= this.activeRoomId	
	var header 		= {t:"sys"}
...
{
	var roomObj = getRoom(roomId)
	
	var header = {t:"sys"}
	var xmlMsg = "<rm id='" + roomObj.getId() + "' />"
...
SmartFoxClient.prototype.connectionEstablished = function()
{
